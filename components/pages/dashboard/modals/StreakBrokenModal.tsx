...
export default function StreakBrokenModal({
  ...
  return (
    <ModalShell
      ...
      onClose={onClose}
      maxWidth="md"
      accentGradient="bg-gradient-to-r from-red-600 via-amber-500 to-red-600"
      errorMsg={errorMsg}
      showCloseButton={false}
      className="border-red-500/20 text-center"
    >
      {/* Animated Cyber Particles */}
      <div className="absolute top-4 left-6 w-2 h-2 rounded-full bg-red-500/20 blur-sm animate-ping pointer-events-none" />
      <div className="absolute bottom-6 right-8 w-3 h-3 rounded-full bg-amber-500/10 blur-sm animate-pulse pointer-events-none" />
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-zinc-400 font-medium leading-relaxed max-w-xs mx-auto">
              Your last streak was <span className="text-red-400 font-extrabold">{previous_streak} days</span> on <span className="text-zinc-200 font-semibold">{formatFullDate(broken_on)}</span>.
            </p>
          </div>
